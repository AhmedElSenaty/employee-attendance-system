...
export const useGetMissionRequests = (
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      missionService.fetchMissionRequests(
        page,
        pageSize,
        startDate,
        endDate,
...
export const useGetMissionRequestsWithAttendance = (
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      QueryKeys.MissionRequests.WithAttendance,
      page,
      pageSize,
      startDate,
      endDate,
    ...
    queryFn: () =>
      leaveService.fetchMissionRequestsWithAttendance(
        page,
        pageSize,
        startDate,
        endDate,
...
export const useGetMyMissionRequests = (
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      missionService.fetchMyMissionRequests(
        page,
        pageSize,
        startDate,
        endDate,
...
export const useGetMissionRequestByID = (missionId: number) => {
  const token = useUserStore((state) => state.token);
  const missionService = useMissionRequestService();
...
export const useCreateMissionRequest = () => {
  ...
  return useMutation({
    mutationFn: (missionData: IMissionRequestCredentials) =>
      missionService.create(missionData),
    onSuccess: ({ status, data }) => {
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.MissionRequests.My],
      });
      if (status === 201) {
...
export const useUpdateMissionRequest = () => {
  ...
  return useMutation({
    mutationFn: (missionData: IMissionRequestCredentials) =>
      missionService.update(missionData),
    onSuccess: ({ status, data }, missionData) => {
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.MissionRequests.My],
      });
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.MissionRequests.MyDetails, missionData.requestId],
      });
      if (status === 200) {
        const message = getTranslatedMessage(data.message ?? "", language);
        showToast("success", message);
...
export const useRejectMissionRequest = () => {
  ...
  return useMutation({
    mutationFn: (missionData: IRejectMissionRequestCredentials) =>
      missionService.reject(missionData),
    onSuccess: ({ status, data }, missionData) => {
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.MissionRequests.All],
      });
      queryClient.invalidateQueries({
        queryKey: [QueryKeys.MissionRequests.Details, missionData.requestId],
      });
      if (status === 200) {
        const message = getTranslatedMessage(data.message ?? "", language);
        showToast("success", message);
