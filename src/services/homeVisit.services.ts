...
import { AssignRequest, BaseSickRequest, CreateRequest, UpdateRequest } from "../interfaces/HomeVisit.interfaces";
...
export class HomeVisitService extends BaseService {
  fetchMyRequests = async(
      ...
      try {
          const params = this.buildParams({
            ...
            const response = await axiosInstance.get("/HomeVisit/Employee/HomeVisits", {
              params,
  ...
  fetchMyRequestById = async(requestId: number) => {
      try{
          const response = await axiosInstance.get(`/HomeVisit/Employee/HomeVisit/${requestId}`, {
              headers: this.getAuthHeaders(),
          });
  ...
  fetchRequestById = async(requestId: number) => {
      try{
          const response = await axiosInstance.get(`/HomeVisit/Manager/HomeVisit/${requestId}`, {
              headers: this.getAuthHeaders(),
          });
  ...
  update = (request: UpdateRequest, userRole: string) => {
		return axiosInstance.put(`/HomeVisit/${userRole}/UpdateRequestHomeVisit`, request, {
				headers: this.getAuthHeaders(),
		});
	}
  ...
  changeToSickRequest = (request: BaseSickRequest, userRole: string) => {
    ...
    return axiosInstance.put(`/HomeVisit/${userRole}/ChangeToSickRequst`, request, 
      {
        headers: {
          ...this.getAuthHeaders(),
