...
interface IDepartmentTableFiltersProps {
  searchBy: string[]
  t: TFunction
}

const DepartmentTableFilters = ({ searchBy, t }: IDepartmentTableFiltersProps) => {
  ...
  return (
    <>
      <div className="bg-white shadow-sm rounded-lg p-4 space-y-4">
        <SectionHeader
          title={t("sectionsHeader.filters.title", { ns: DEPARTMENT_TRANSLATION_NAMESPACE })} 
          description={t("sectionsHeader.filters.description", { ns: DEPARTMENT_TRANSLATION_NAMESPACE })} 
        />
        <div className="flex gap-3">
          ...
          {/* Search Type */}
          <Field className="space-y-3 w-full sm:w-auto flex flex-col">
            <Label size="md">{t("searchBy.label", { ns: DEPARTMENT_TRANSLATION_NAMESPACE })}</Label>
            <SelectBox value={searchKey ?? ""} onChange={(e) => setFilters({ searchKey: e.target.value })} >
              <option value="">
                {t(`searchBy.default`, { ns: DEPARTMENT_TRANSLATION_NAMESPACE })}
              </option>
              {searchBy.map((search, idx) => (
                <option key={idx} value={String(search)}>
                  {t(`searchBy.${String(search)}`, { ns: DEPARTMENT_TRANSLATION_NAMESPACE }) ?? ""}
                </option>
              ))}
            </SelectBox>
          </Field>
...
export default DepartmentTableFilters