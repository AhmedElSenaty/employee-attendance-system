...
interface IProfileTableFiltersProps {
  searchBy: string[]
  t: TFunction
}

const ProfileTableFilters = ({ searchBy, t }: IProfileTableFiltersProps) => {
  ...
  return (
    <>
      ...
      {/* Search Type */}
      <Field className="space-y-3 w-full sm:w-auto flex flex-col">
        <Label size="md">{t("manageProfilesPage.searchBy.label", { ns: PROFILE_TRANSLATION_NAMESPACE })} </Label>
        <SelectBox value={searchKey ?? ""} onChange={(e) => setFilters({ searchKey: e.target.value })} >
          <option value="">
            {t(`manageProfilesPage.searchBy.default`, { ns: PROFILE_TRANSLATION_NAMESPACE })}
          </option>
          {searchBy.map((search, idx) => (
            <option key={idx} value={String(search)}>
              {t(`manageProfilesPage.searchBy.${String(search)}`, { ns: PROFILE_TRANSLATION_NAMESPACE }) ?? ""}
            </option>
          ))}
        </SelectBox>
      </Field>
