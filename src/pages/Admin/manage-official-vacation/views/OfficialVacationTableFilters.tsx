...
const OfficialVacationTableFilters = ({ searchBy, t }: IOfficialVacationTableFiltersProps) => {
  ...
  return (
    <>
      ...
      {/* Search Type */}
      <Field className="space-y-3 w-full sm:w-auto flex flex-col">
        <Label size="md">{t("searchBy.label", { ns: OFFICIAL_VACATIONS_TRANSLATION_NAMESPACE })} </Label>
        <SelectBox value={searchKey ?? ""} onChange={(e) => setFilters({ searchKey: e.target.value })} >
          <option value="">
            {t(`searchBy.default`, { ns: OFFICIAL_VACATIONS_TRANSLATION_NAMESPACE })}
          </option>
          {searchBy.map((search, idx) => (
            <option key={idx} value={String(search)}>
              {t(`searchBy.${String(search)}`, { ns: OFFICIAL_VACATIONS_TRANSLATION_NAMESPACE }) ?? ""}
            </option>
          ))}
        </SelectBox>
      </Field>
...
export default OfficialVacationTableFilters