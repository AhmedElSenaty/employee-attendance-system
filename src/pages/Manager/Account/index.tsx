...
import { RenderPermissions } from "../../Admin/Account/views";
...
const ManagerAccountPage = () => {
  ...
  const handleConfirmEdit: SubmitHandler<IManagerCredentials> = async (
    request
  ) => {
    if (me) {
      request.id = me.id;
  ...
  const { mutate: updateMyPassword, isPending: isUpdateMyPasswordLoading } =
    useUpdateMyPassword();

  const handleConfirmUpdatePassword: SubmitHandler<{
    password: string;
    oldPassword?: string;
  }> = async (request: { password: string; oldPassword?: string }) => {
    updateMyPassword({
  ...
  const renderPermissions = me?.permissions?.map(
    ({ id, nameAr, nameEn }: IPermissionsData) => (
      <div
        key={id}
  ...
  return (
    <div className="sm:p-5 p-3 space-y-5">
      ...
      <div className="bg-white shadow-md space-y-5 p-5 rounded-lg">
        <SectionHeader
          title={t(`profile.updateSection.title`, {
            ns: TRANSLATION_NAMESPACE,
          })}
          description={t(`profile.updateSection.description`, {
            ns: TRANSLATION_NAMESPACE,
          })}
        />
        ...
        <form className="space-y-5" onSubmit={handleSubmit(handleConfirmEdit)}>
          <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
            <Field className="space-y-2">
              <Label size="lg">
                {t(`form.username.label`, { ns: TRANSLATION_NAMESPACE })}
              </Label>
              <Input
              ...
              {errors["username"] && (
                <InputErrorMessage>
                  {t(
                    `form.username.inputValidation.${errors["username"].type}`,
                    { ns: TRANSLATION_NAMESPACE }
                  )}
                </InputErrorMessage>
              )}
            ...
            <Field className="space-y-2">
              <Label size="lg">
                {t(`form.email.label`, { ns: TRANSLATION_NAMESPACE })}
              </Label>
              <Input
      ...
      <form
        className="bg-white shadow-md space-y-5 p-5 rounded-lg w-full"
      ...
      >
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
          <Field className="space-y-2">
            <Label size="lg">
              {t(`form.oldPassword.label`, { ns: TRANSLATION_NAMESPACE })}
            </Label>
            <Input
            ...
            {updatePasswordErrors["oldPassword"] && (
              <InputErrorMessage>
                {t(
                  `form.oldPassword.inputValidation.${
                    updatePasswordErrors["oldPassword"].type === "matches"
                      ? updatePasswordErrors["oldPassword"].message
                      : updatePasswordErrors["oldPassword"].type
                  }`,
                  {
                    ns: TRANSLATION_NAMESPACE,
                  }
          ...
          <Field className="space-y-2">
            <Label size="lg">
              {t(`form.password.label`, { ns: TRANSLATION_NAMESPACE })}
            </Label>
            <Input
          ...
          <Field className="space-y-2">
            <Label size="lg">
              {t(`form.confirmPassword.label`, { ns: TRANSLATION_NAMESPACE })}
            </Label>
            <Input
            ...
            {updatePasswordErrors["confirmPassword"] && (
              <InputErrorMessage>
                {t(
                  `form.confirmPassword.inputValidation.${updatePasswordErrors["confirmPassword"].type}`,
                  { ns: TRANSLATION_NAMESPACE }
                )}
              </InputErrorMessage>
            )}
        ...
        <div className="flex flex-wrap gap-3">
          <Button fullWidth={false} isLoading={isUpdateMyPasswordLoading}>
      ...
      <div className="bg-white shadow-md space-y-5 p-5 rounded-lg w-full">
        <SectionHeader
          title={t(`profile.departmentsSection.title`, {
            ns: TRANSLATION_NAMESPACE,
          })}
          description={t(`profile.departmentsSection.description`, {
            ns: TRANSLATION_NAMESPACE,
          })}
        />
        <div className="flex flex-wrap gap-5 mt-4 w-fit">
          <div className="p-3 bg-primary text-white shadow-md rounded-lg flex gap-4 justify-between items-center">
            <div className="flex-1">
              <h3 className="text-base font-semibold">
                {me?.department?.name}
              </h3>
