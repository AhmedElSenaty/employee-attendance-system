...
const OrdinaryRequestsPage = () => {
  ...
  const { ordinaryRequest, isOrdinaryRequestLoading } = useGetOrdinaryRequestByID(selectedID);

  const { mutate: acceptOrdinaryRequest, isPending: isAccepting } = useAcceptOrdinaryRequest()
  const { mutate: rejectOrdinaryRequest, isPending: isRejecting } = useRejectOrdinaryRequest()
  ...
  const handleConfirmReject = handleSubmit((request: IRejectOrdinaryRequestCredentials) => {
