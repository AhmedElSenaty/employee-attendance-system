...
const SubDepartmentTableFilters = ({ searchBy, t }: ISubDepartmentTableFiltersProps) => {
  ...
  return (
    <>
      ...
      {/* Search Type */}
      <Field className="space-y-3 w-full sm:w-auto flex flex-col">
        <Label size="md">{t("searchBy.label", { ns: SUB_DEPARTMENT_TRANSLATION_NAMESPACE })} </Label>
        <SelectBox value={searchKey ?? ""} onChange={(e) => setFilters({ searchKey: e.target.value })} >
          <option value="">
            {t(`searchBy.default`, { ns: SUB_DEPARTMENT_TRANSLATION_NAMESPACE })}
          </option>
          {searchBy.map((search, idx) => (
            <option key={idx} value={String(search)}>
              {t(`searchBy.${String(search)}`, { ns: SUB_DEPARTMENT_TRANSLATION_NAMESPACE }) ?? ""}
            </option>
          ))}
        </SelectBox>
      </Field>
...
export default SubDepartmentTableFilters