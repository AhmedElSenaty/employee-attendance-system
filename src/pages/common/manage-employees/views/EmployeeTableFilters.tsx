...
const EmployeeTableFilters = ({ searchBy = [], t }: IEmployeeTableFiltersProps) => {
  ...
  return (
    <>
      ...
      {/* Search Type */}
      <Field className="space-y-3 w-full sm:w-auto flex flex-col">
        <Label size="md">{t("manageEmployeesPage.searchBy.label", { ns: EMPLOYEE_TRANSLATION_NAMESPACE })} </Label>
        <SelectBox value={searchKey ?? ""} onChange={(e) => setFilters({ searchKey: e.target.value })} >
          <option value="">
            {t(`manageEmployeesPage.searchBy.default`, { ns: EMPLOYEE_TRANSLATION_NAMESPACE })}
          </option>
          {searchBy.map((search, idx) => (
            <option key={idx} value={String(search)}>
              {t(`manageEmployeesPage.searchBy.${String(search)}`, { ns: EMPLOYEE_TRANSLATION_NAMESPACE }) ?? ""}
            </option>
          ))}
        </SelectBox>
      </Field>
